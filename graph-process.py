...
def knn_graph(point_cloud, k, output_file):
    num_points = len(point_cloud)
    
    # Initialize a sparse adjacency matrix
    adjacency_matrix = lil_matrix((num_points, num_points), dtype=float)  # Use float type for weights
...
k = 10
# Output file name
#output_file = "/media/jacob/maxone/PCN-Jet-Tagging-master/knn_graph.pkl"
# Construct the KNN graph with weights and save as an edge list
#knn_graph(x_jets, k, output_file)

# Load the graph from the edge list file into DGL
...
dir = "G:/PCN-Jet-Tagging-master/data/test/"
def main():
    files = [filename for filename in os.listdir(dir)]
    for file in tqdm(files, desc="Processing files", unit="file"):
        filename = dir + file
        x_particles, x_jets, y = read_file(filename)
        k = 10
        output_file = filename.rstrip(".root") + ".pkl"
        knn_graph(x_jets, k, output_file)
        adjacency_matrix = load_adjacency_matrix(output_file)
    print("done")
